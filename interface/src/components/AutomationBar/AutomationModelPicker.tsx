...
export function AutomationModelPicker({
  ...
  const { model: effectiveModelId, setModel } = useAutomationModel(projectId);
  // `effectiveModelId` is `null` until the user (or the persisted
  // fallback chain in `useAutomationModel`) yields a pick. Pass `""`
  // so `modelLabel` falls through to `DEFAULT_MODEL.label` instead of
  // throwing — the picker still renders a sensible default label.
