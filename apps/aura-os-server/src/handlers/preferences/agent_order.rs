...
/// `projects_app` / `tasks_app` map project_id → ordered agent_id list.
/// `None` means "inherit from agents_app" for all projects on that surface.
...
pub struct AgentOrderPrefs {
    ...
    #[serde(default)]
    pub tasks_app: Option<HashMap<String, Vec<String>>>,
}
