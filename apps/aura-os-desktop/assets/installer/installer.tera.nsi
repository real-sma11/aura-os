; ============================================================================
; AURA installer.tera.nsi
;
; Vendored from crabnebula-dev/cargo-packager
;   crates/packager/src/package/nsis/installer.nsi @
;     37a538e76608b33eaa3f36f7c57b30b284dfa5a9
;
; NOTE on filename: cargo-packager actually renders this template via
; HANDLEBARS (see crates/packager/src/package/nsis/mod.rs - it constructs a
; Handlebars instance, not Tera). The `.tera.nsi` suffix is preserved purely
...
;   2. Reapply every hunk marked with `; AURA:` in this file
;   3. Bump the commit hash in this header
; ============================================================================
...
Function .onInit
  ...
  ; ========================================================================
  ; AURA: Windows light/dark theme detection + bitmap swap.
  ;
  ; Read AppsUseLightTheme from HKCU. The value is missing on Win10 < 1809
  ; and on locked-down enterprise images; in that case we default to light
  ...
  ; are extracted automatically at startup from the bitmaps embedded via
  ; MUI_HEADERIMAGE_BITMAP / MUI_WELCOMEFINISHPAGE_BITMAP defines above.
  ; If a future MUI revision changes those names, the runtime swap in
  ; AuraThemedWelcomeShow below acts as a fallback for the most visible
  ; surface (the welcome/finish sidebar).
  ; ========================================================================
  InitPluginsDir
  ...
  ${If} $R8 != "0"
    ; Light mode: overwrite MUI's pre-extracted dark bitmaps with the light
    ; variants. This is best-effort; the runtime LoadImageW swap in
    ; AuraThemedWelcomeShow below handles the case where MUI uses
    ; different filenames or has already cached the bitmap handle.
    CopyFiles /SILENT "$PLUGINSDIR\header-light.bmp"  "$PLUGINSDIR\modern-header.bmp"
    CopyFiles /SILENT "$PLUGINSDIR\sidebar-light.bmp" "$PLUGINSDIR\modern-wizard.bmp"
  ${EndIf}
FunctionEnd
...
; AURA: belt-and-suspenders runtime bitmap swap for the welcome / finish
; page sidebar. Runs every time either page is shown; replaces the bitmap
; on control id 1201 (the standard MUI welcome/finish sidebar STATIC
; control) with the light variant when the user is in light mode. Also
; reapplies the dark title bar to the inner dialog so it matches the
; outer wizard frame.
Function AuraThemedWelcomeShow
  ${If} $R8 != "0"
    ...
    ${If} $0 != 0
      ...
      ${If} $1 != 0
        ...
        System::Call 'user32::LoadImageW(p 0, w "$PLUGINSDIR\sidebar-light.bmp", i 0, i 0, i 0, i 0x10) p .r2'
        ${If} $2 != 0
          ; STM_SETIMAGE = 0x0172, IMAGE_BITMAP = 0
          SendMessage $1 0x172 0 $2
        ${EndIf}
      ${EndIf}
    ${EndIf}
  ${Else}
    FindWindow $0 "#32770" "" $HWNDPARENT
    ${If} $0 != 0
      System::Call 'dwmapi::DwmSetWindowAttribute(p $0, i 20, *i 1, i 4) i .r1'
    ${EndIf}
  ${EndIf}
FunctionEnd
