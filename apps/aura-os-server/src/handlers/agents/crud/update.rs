...
async fn invalidate_chat_sessions_for_agent(state: &AppState, agent_id: &AgentId) {
    // Phase 4: chat_sessions is a DashMap keyed on
    // `(session_key, model)`. The sweep matches on `template_agent_id`
    // and drops each entry; see
    // [`AppState::evict_chat_sessions_for_agent`] for the lock-light
    // collect-then-remove contract shared with the remote-VM
    // recovery / lifecycle paths.
